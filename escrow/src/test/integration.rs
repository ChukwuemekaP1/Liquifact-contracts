...
fn test_escrow_gold_standard_happy_path_open_overfund_snapshot_settle_claim() {
    ...
    // Bob contributes 40,000 USDC (80% of target) - this triggers funding completion AND overfunding
    let bob_amount = 40_000 * USDC_DECIMALS;
    ...
    assert_eq!(
        snapshot.closed_at_ledger_timestamp,
        env.ledger().timestamp(),
        "Timestamp must match current ledger"
    );
    assert_eq!(
        snapshot.closed_at_ledger_sequence,
        env.ledger().sequence(),
        "Sequence must match current ledger"
    );
